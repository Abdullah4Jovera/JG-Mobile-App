...
router.post('/apply-for-personal-loan', isAuth , isUser, async (req, res) => {
  ...
  router.put('/upload-documents/:loanId', isAuth, isUser, async (req, res) => {
    try {
        ...
        // Ensure personalLoan.documents is an array
        if (!Array.isArray(personalLoan.documents)) {
            personalLoan.documents = [];
        }

        // Append new document URLs to existing documents
        personalLoan.documents.push(...documents);
