...
router.post("/uploads", upload.array('documents', 10), async (req, res) => {
    try {
        ...
        res.status(200).json({  secure_urls }); // Send JSON response
    } catch (error) {
        console.error('Error uploading documents:', error);
        res.status(500).json({ error: true, message: "Internal server error", details: error.message }); // Send JSON response
    }
});
